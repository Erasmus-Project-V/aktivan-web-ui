    ...
    function onChooseGrade() {
        console.log("Grade chosen");
    }

    function onShowOptions(event: MouseEvent) {
...
<div
    on:click={onChooseGrade}
    role="button"
    tabindex="0"
    on:keydown={(e) => {
        if (e.key === "Enter") onChooseGrade();
    }}
...
>
    ...
    <button on:click={onShowOptions} class="absolute top-4 right-7">
        <ElipsisIcon />
    </button>
</div>
